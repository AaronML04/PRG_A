...
public class UtilitatsMatrius {
    ...
    public static int[] generaFila(int[][] matriu, int y) {
    int[] fila = new int[matriu[0].length];
    ...
    int[] columna = new int[matriu.length]; // tantas filas como la matriz
    for (int i = 0; i < matriu.length; i++) {
        columna[i] = matriu[i][y]; // toma el elemento de la columna y
    }
    return columna;
    }
}
